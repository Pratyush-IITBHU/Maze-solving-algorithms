...
class DFS(grid_construction_manipulation):
	...
	def evaluate(self,draw, grid, start, end):
		...
		distance_param = []
		for x in neigbours:
			distance_param.append(self.distance_function(x.position_row_col(), end.position_row_col()))
		...
		for event in p.event.get():
			...
			if event.type == p.KEYDOWN:
				if event.key == p.K_SPACE and start and end:
					...
					a_star.evaluate(lambda: class_grid.draw(pygame_window, grid, rows, width_of_window), grid, start, end)
